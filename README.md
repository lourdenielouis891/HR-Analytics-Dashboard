 📊 HR Analytics Dashboard (Excel)

## 📌 Project Overview
This project analyzes HR data to uncover workforce trends, employee attrition, and department performance. The goal is to provide insights that support better HR decision-making.


 ##🛠️ Tools Used
- Microsoft Excel
- Pivot Tables
- Data Visualization (Bar Charts, Pie Charts)
- Data Cleaning & Filtering
  


## 📊 Dashboard Preview

![Dashboard Screenshot](https://github.com/lourdenielouis891/HR-Analytics-Dashboard/blob/main/Screenshot%202026-05-03%20235632.png?raw=true)
                                           Key visuals showing employee status and workforce composition.


## 📁 Dataset
HR dataset including employee status, department, and employment type.

Source: Internal HR dataset (simulated for analysis purposes)                                    

## 📈 Key Insights
- The majority of employees are active (86%), indicating strong workforce retention
- The Production department has the highest number of employees
- Full-time employees represent the largest workforce segment
- Termination rate is relatively low compared to active employees


 ##💡 Business Insights
- HR teams can focus on departments with lower headcount for potential growth
- Monitoring termination trends can help improve retention strategies
- Workforce composition shows a strong reliance on full-time employees


 ##🎯 Skills Demonstrated
- Data Cleaning
- Data Analysis
- Data Visualization
- Dashboard Creation
- Business Insight Generation
